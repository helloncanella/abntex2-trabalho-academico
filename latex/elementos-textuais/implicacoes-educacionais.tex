...
Falando especificamente de modelos, podemos dizer que eles ``todos estão errados, mas alguns deles são úteis'' \apud{box1987empirical}{Weintrop2016}, dado o caráter simplificador, mas representativo de alguns - tópico extensivamente explorado nos capítulos anteriores.

Dentre as várias formas que um modelo pode assumir, o autor destaca, os fluxogramas, diagramas, equações, fórmulas químicas, modelos físicos (como maquetes ou representações tridimensionais de uma molécula), e por fim os modelos computacionais. 
...
Já no caso de simulações... (FALAR SOBRE)

De um ponto de vista pedagógico, como ressalta o autor, a utilidade de modelos e simulações computacionais está na possibilidade de tornar compreensão de um fenômeno mais clara para o estudante, ao mesmo tempo que permite a ele a investigação de comportamentos cuja condições determinantes são difíceis senão impossíveis de reproduzir. 

O poder didático dessas ferramentas, contudo, não está apenas no seu uso, mas na possibilidade do estudante também criá-las, tornando possível a ele a formulação de suas próprias interpretações. 
...
% Contudo, o poder dessas ferramenta didática se concentra de fato na possibilidade do aluno não apenas o aluno usar modelos, mas poder cria-los


% Como lembra o autor, a utilidade de um modelo advém essencialmente da sua capacidade preditiva ou explicatória
